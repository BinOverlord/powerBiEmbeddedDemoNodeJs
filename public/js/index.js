...
async function alertFilters(){
    try {
        ...
        let visual = visuals.filter(function (visual) {
            return visual.name === "VisualContainer8";
        })[0];
...
report.on("dataSelected", function (event) {
    ...
    event.detail.dataPoints.forEach((dataPoint) =>{
        text = dataPoint.identity[0].target.table + " " + dataPoint.identity[0].equals 
    })
    window.alert(text);
});
try {
