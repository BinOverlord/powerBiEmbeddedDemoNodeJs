 ...
 */
 async function printReport() {
    ...
    // Get report info by calling the PowerBI REST API
    const result = await fetch(reportInGroupApi + "/ExportTo", {
        method: 'POST',
        headers: headers,
        body: JSON.stringify(formData)
    })
    ...
    let resultJson2;
    do {
        let result2 = await fetch(reportInGroupApi + "/exports/" + resultJson.id, {
            method: 'GET',
            headers: headers,
        })
    
    
        resultJson2 = await result2.json();
    
        console.log(resultJson2.status)

        await delay(5000);

    } while(resultJson2.status == "Running")


    let result3 = await fetch(reportInGroupApi + "/exports/" + resultJson.id + "/file", {
        method: 'GET',
        headers: headers,
    })
    ...
    await streamPipeline(result3.body, createWriteStream('./report.pdf'));
    //let resultJson3 = await result3.json();
    return result3.body;
}
